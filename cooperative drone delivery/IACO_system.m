...
while iter<=iter_max    
    ...
    for j=2:n
        for i=1:m
            tabu=table(1:((j-1)*4+i-1));  %把第i只蚂蚁之前所走过的所有node放入禁忌表中 【把table里面从第一个数到j-1全放到禁忌表中】
    ...
    %吧wholetable总共40行，先每行进行l1  and t1之后，再更新信息素
     s1=wholetable;
    ...
    Length1=zeros(4,1);
    for i=1:4
        route1=wholetable2((i+h),:);  %【把第i只蚂蚁走过的路径给route】【route里就是m只蚂蚁每只走过的序列】
        for j=1:5
