...
for i=1:M
    Path1(i,1)=(rand-F1)*20;
    Path2(i,1)=(rand-F2)*20;
    Path3(i,1)=(rand-F3)*20;
    for j=2:D
        Path1(i,j)=Path1(i,j-1)+(rand-F1)*10;
        Path2(i,j)=Path2(i,j-1)+(rand-F2)*10;
        Path3(i,j)=Path3(i,j-1)+(rand-F3)*10;
    end %初始化横坐标
end
tic
%% 迭代计算
